...
/* 작업순서
    ...
    5. (로직)
        1. 현재 회원들 중에서 구독중인 회원찾기
            ...
            (2) 하나씩 조회중인 회원정보가 구독로그테이블(배열) 내 '회원코드'가 일치한 정보 확인
            (3) 일치한 회원코드가 있으면(구독중인 상태) '구독중' 
                일치한 회원코드가 없으면(구독중이 아닌상태) '구독 내역이 없습니다.'
                라고 document.write()에 출력하시오. 
            (4) 구독내역이 있을때 구독중인 구독상품명과 가격을 찾아서 '상품명(가격)' 형식으로 출력하시오.
...
// 구독로그에 화면코드가 존재하지 않으면 구독 내역이 없다.                  
...
for (let i=0; i<=회원테이블.length-1; i++){
    ...
    if(구독상태 == true){ //구독내역 있음
        for(let k=0; k<=구독상품테이블.length-1; k++){
            let 상품정보=구독상품테이블[k];
            if( 상품정보.상품코드==구독중인상품코드){
                html += `<ul> 
