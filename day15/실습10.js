...
for(i=0;i<=학생정보.length-1;i++){
    ...
    for(j=0;j<=수강신청내역정보.length-1;j++){
        const 수강신청=수강신청내역정보[j];         
        if(학생.학번==수강신청.학번){
            for(k=0;k<=과목정보.length-1;k++){
                const 과목=과목정보[k];
                if( 과목.과목코드 == 수강신청.과목코드){
                    document.write( `<h4> --> ${과목.과목명} (${과목.교수자명})</h4>`)
                    document.write( `<h5> --> 성적 : ${수강신청.성적}</h5>`)
                    document.write( `<h5> --> 이수여부 : ${수강신청.성적== 'F' ? 'fail' : 'pass'}</h5>`)
                }
