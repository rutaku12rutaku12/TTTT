...
4. 핵심 기능 구현
    ...
    -login() 구현:
        ...
        --일치하는 정보를 찾지 못하면, "동일한 회원정보가 없습니다. 로그인 실패" 알림창을 띄웁니다.
 */




// ======================================== (1) 데이터 구성, 전역변수 대신에 ssesionStorage ============================= //
// const memberList = [ { no : 1 , id : 'qwe' , pw : 'qwe' } ,
//                      { no : 2 , id : 'asd' , pw : 'asd' } 
// ]
// ======================================== (2) 회원가입 함수 ============================= //
function signup(){ console.log('--> signup exe');
    ...
    let no = 1; // 회원번호 초기값
        ...
        if( memberList == null ){ // 해당 속성명(memberList)이 존재 하지 않으면
            ...
            no = memberList[ memberList.length-1].no +1;      // 배열내 마지막 인덱스의 회원번호 + 1
        }
...
function login(){console.log('--> login exe');
    ...
    for(let i=0; i<=memberList.length-1; i++){
        ...
        if(member.id == id && member.pw == pw ){
            // 만약에 i번째 회원아이디가 입력한 아이디와 같고 회원비밀번호가 입력한 비밀번호 같으면
            alert("로그인 성공");
             return; // 강제 함수(반복문) 종료
        }
    } // for end
