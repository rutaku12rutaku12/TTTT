...
function getBoard(){ // JS가 실행될때 최초1번 자동으로 함수실행 
    ...
    for( let index=0; index <= boardList.length-1; index++){
        ...
        if ( obj.no == selectNo ){ // 만약에 내가 선택한 게시물번호와 일치하면
            // 5. 찾았으면 출력 
...
function boardDelete(){
    ...
    for( let i =0; i< boardList.length; i++){
        ...
        if( obj.no == selectNo ){ // 만약에 선택된 게시물번호를 찾았으면
            // 5. 확인용 비밀번호를 입력받아 기존 비림번호와 일치하면 삭제, 아니면 실패
...
function boardUpdateView(){
    ...
    // 5. 확인용 비밀번호를 입력받아 기존 비림번호와 일치하면 수정페이지 이동
