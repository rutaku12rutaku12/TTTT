...
console.log( 10 - 3 ); // 7
console.log( 10 * 3 ); // 30
console.log( 10 / 3 ); // 3.333333333333333335
...
let 원넓이 = 반지름 * 반지름 * 3.14;    //처리
console.log( `원넓이 : $( 원넓이 )`);  //출력 */

// [지문3] prompt 함수로 두 실수를 입력받아서 앞 실수의 값이 뒤실수의 값의 비율% 계산하여 console탭에 출력하시오.

/* let 상수1 = Number( prmopt("실수1 : "));
let 상수2 = Number( prmopt("실수2 : "));
let 비율 = ( 상수1/상수2 )*100; */     // 30 이 100에서 몇퍼센트(비율)인지 => 30%
// 30 의 50%를 구해라 30 * 0.5
...
/* 
    && 이면서 면서 이고 그리고 모두 and조건 , 조건이 모두 ture 이면 true  
    || 이거나 거나 하나라도 또는 or 조건 , 조건이 하나라도 true 이면 true * | : + [쉬프트] 엔터 위에 원화기호
    ! 부정 반대 not 조건 , true -> false , false -> ture
        초등학생이 사탕 이면서 초콜릿 먹는다. and조건 , 2개다 먹어야 true(충족)
        초등학생이 사탕 이거나 초콜릿 먹는다. or조건 , 1개다 먹어야 true(충족)
        초등학생이 사탕 아니고 , not 조건 , 사탕이 아닌
        *초범위논리 지원하지 않는다. 5 < 점수 <10 ------> 점수 > 5 && 점수 < 10
*/


// [지문4] prompt 함수로 정수를 입력받아 입력받은 값이 홀수이면 true / 짝수이면 false 로 console탭에 출력하시오.
    // 홀/짝 계산식 : 값이 나누기 2를 했을때 나머지가 1이면 홀수이고 0이면 짝수 , 값% 2 == 1 , 값 % == 0 
/* 
let 정수 = Number(prompt("정수: "))     //  10         7
let 결과 = 정수 % 2 == 1;
console.log( `결과 : ${ 결과 }`)        // false     ture 
 */
// [지문5] prompt 함수로 정수를 입력받아 입력받은 값이 7의 배수이면 true / 아니면 false 로 console탭에 출력하시오.
    // 배수 계산식 : 값이 나누기 수 를 했을때 나머지가 0이면 해당 값은 그 수의 배수값 , 값 % == 0
/* let 정수 = prompt( " 정수: " )
let 결과 = 정수 % 7 == 0
console.log( `결과 : ${결과}`) 
 */
...
/*
    ...
    += : 오른쪽 데이터를 왼쪽에 더한 후에 대입/할당/놓기       , Let A = 2 , A = A + 3 vss A += 3    
    -=, *=, /= , %=
*/
...
console.log(나이);     42
--나이; // 증감연산자 외 다른 명령어가 없으면 선위/후위 가 상관없다. *세미클론 단위 우선순위 따짐.
나이--; */

// 7. **삼항연산자** , 항이 3개 , 조건 ? (2)참일때 : (3)거짓일때 , true와 false 대신 다른값 대체 
// 조건 ? 참 : 거짓
...
// [지문8] 1차점수 와 2차점수 prompt함수로 각 입력받아서 총점이 150점이상이면 '합격' 아니면 '불합격' HTML의 <h3> 에 출력하시오.
    ...
    let _1차점수 = prompt("1차점수: ");                      //변수명은 숫자로 시작 불가능
    let _2차점수 = prompt("2차점수: ");                      //prompt 는 항상 문자열로 반환/결과 준다.
    let 총점 = Number(_1차점수) + Number(_2차점수); 
    합격여부 = 총점 >= 150 ? "합격" : "불합격";             // 논리는 항상 true와 false로 답한다. 삼항 또는 조건문
    console.log(`총점 : ${총점} , 합격여부 : ${합격여부}`);  // true 대신에 '합격' , false 대신에 '불합격'
    document.write( `<h3>${합격여부}</h3>`);
    // [지문9] 두 사람의 이름을 prompt함수로 각 입력받아서 만일 이름이 '유재석' 이면 뒤에 (방장) 이라고 이름 뒤에 붙이고 아니면 생략한다.  HTML의 <ol> 에 결과를 출력하시오.

let 이름 = prompt("이름 : ")
let result2 = 이름 == "유재석" ? `${이름}(방장)` : `${이름}`// *변수명은 중복이 불가능하다.* // 논리는 항상 ture와 false 로 답한다.
            //    (조건)      ?       참        :  거짓
