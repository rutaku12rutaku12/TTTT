...
// 1. 정수 : 소수점 없는 숫자
console.log(100)
console.log( typeof 100) // typeof 자료 : 자료의 자료형(종류) 반환/도출

// 2. 실수 : 소수점 없는 숫자 : (부동소수점)정수는 정밀하게 계산이 불가능하다. -> 별도의 정밀한 함수 필요하다.
console.log(0.1+ 0.2)

// [문자열] : ''또는 "" 묶음 자료
console.log( typeof "100" ) // 숫자도 ""로 묶으면 문자열 이다.
console.log( typeof "안녕하세요" )
// 1. 이스케이프문자 : 기능이 포함된 특수문자 , \백슬래시 : 키보드 엔터 위에 원화기호\
console.log( "\\안녕1" ) // \\ : 백슬래시 출력
console.log( "\'안녕2" ) // \' : '작은 따옴표 출력   
console.log( "\"안녕3" ) // \" : "큰 따옴표 출력
console.log( "\n안녕4" ) // \n : 줄 바꿈 처리
...
let 채소상자 = null;    // 변수에 null이 대입/저장 된 상태 , 라이브러리 함수들이 null 반환하는 경우가 많다. 
console.log( 채소상자 ) // 변수에 null 일 경우에는 null ( 자료가 존재 또는 유효하지 않다. )
...
console.log( season[4] )    // undefined , 존재하지 않은 인덱스 이므로
    // * [생각해보기1] 
