...
[함수 , function ]
    ...
    2. 주 목적 : 미리 정의된 코드를 재사용
        - 변수 : 값/데이터 저장 해서 재사용                     , 객체명.이름 : 자료호출
        - 반복문 : 특정한 패턴(조건) 에 따른 코드 반복(재사용)
        - 함수 :  미리 정의된 코드 에 따른 코드 반복             , 객체명.이름() : 함수호출 // "()소괄호가 뒤에있으면 함수"
    3. 함수 종류
        (1) 라이브러리 : 미리 만들어진 함수 , < 사용 > 
            - console.log( ) , document.write() , push() , splice() 등등
            - 공부법 : 이해가 아닌 필요한 상황에 익히기 
    ...
    6. 함수 만드는 방법
        1. foncion      : 함수를 선언(만들기) 하기 위한 키워드
        2. 함수명        : 함수명 과 동일하게 카멜표기법 권장, 숫자로 시작 X , 띄어쓰기 X  
...
내가만든함수( );
    ...
    // cl("할머니");
    // function forI(x){for(i=0;i<=x.length-1;i++)}
    // cl(forI(x){"1"})
...
/*  
    console.log();      [x] -> log() 출력후 반환되는 값이 없으므로
