...
let currentPno = 3; // * 코드를 자동대입 하기 위한 현재 코드번호 , 샘플의 마지막 코드번호 로 초기화
// * 확인차
console.log(categoryList);
console.log(productList);
// ======================== [2] 데이터 모델링 샘플 =================== //
// 1. 카테고리 출력함수 : 실행조건 : 페이지(HTML/JS) 가 열렸을때 실행 카테고리목록의 객체정보를 <select>에 출력하는 함수
categoryPint();
...
function productPrint(){ console.log('>>productPrint() exe')
    ...
    let html = '';
        for( let index = 0; index<= productList.length-1 ; index++){
            ...
            const caetgory = getCategory(product.cno);
            html += `<tr>
                        <td> <img src="${product.pimg}" /> </td>   
                        <th> ${getCategory(product.cno).cname} </td>   
                        <td> ${product.pname} </td> 
...
function productDelete( pno ){console.log( '>>productDelete exe '); console.log( pno );
    ...
    for( let index = 0; index <= productList.length-1 ; index++){
        if( productList[index].pno == pno){ // 만약에 index번째 제품코드와 삭제할제품코드가 같으면
            productList.splice( index , 1 ) // 해당 index 해서 요소1개 삭제
            alert('[성공] 제품 삭제 '); // 안내
...
function productEdit( pno ) { console.log( '>>productEdit exe'); console.log( pno ) ;
    for( let index = 0; index <= productList.length-1 ; index++){  // (1) 수정할 번호의 객체를 찾는다. <for>
