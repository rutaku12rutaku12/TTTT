...
// for( let i = 1; i <=2 ; i++){ //  ---------------  한달간 반복 해석 서술 : i는 1부터 2이하까지 1씩증가 --------------------
//     console.log(i) ;
//     for ( let h = 1 ; h <=3 ; h++ ){    // h는 1부터 3이하 까지 1씩증가
//         console.log(h)
//     }
// } // for end
...
// [3] 구구단 : 단 안(마다)에 곱 포함 인가?? 곱 안(마다)에 단 포함 인가??
...
/* 순서도 
    ...
    단 = 3      ture            곱 =1       true            console.log출력     곱++ 
    ~~~~~~~
    단 = 10     false
...
/*              line(row)       stra(column)
    ...
    *****           5               12345
    line 1부터 5까지 1씩 증가
    star 1부터 현재줄(1/2/3/4/5) 까지 1씩 증가 , (1/2/3/4/5) , 현재줄수(line)
*/
// let output = ""; // 빈 문자열 선언                                                      output = ""
// for( let line = 1 ; line <= 5 ; line++ ){ // ---행(line), 5번
//     // line 1부터 5이하까지 1씩증가
//     for( let star = 1 ; star<= line ; star++){ // ---열(row) 
//         output += "*"; // 별 찍기                                                       
//     }    // 별 for end
//     output += "\n"; // \n : 이스케이프문자( 줄바꿈처리 )
// } // 열 for end
// console.log(output); // output  "*/n**/n***/n****/n******/n"

// [ 반복문 제어 키워드 ]
// (1) continue;    : 가장 가까운 for{}문의 증감식으로 (코드흐름)이동
    // 1부터 5까지 출력, 단) 3이면 출력하지 않는다.
// for( let i = 1 ; i <= 5 ; i++){
//     if( i == 3 ){continue;} // i가 3일때 continue; 이므로 아래 console 생략안됨. 
...
//     sum += i; // 누적합계 vs sum = sum + 1
// }
// console.log(sum); // 15

// (3) 무한루프         : for( ; ; ){}
// for ( ; ; ){console.log("무한루프중");}
...
//     if( num <= 0 || num > 45){ // 1보다 작거나 45보다 초과이면 
//         console.log('[경고] 1~ 45사이만 입력해주세요.')
//         continue; // 코드 흐름을 현재 가까운 for{} 증감식으로 이동 
//     }
...
//     if( numList.length == 6 ){ // 변수명.length : 현재 배열의 전체 자료수 == 총 길이 }
//         break; // 코드 흐름을 현재 가장 가까운 for{} 끝낸다.
//     }
// } // for end
