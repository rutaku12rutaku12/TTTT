/* 
    객체란 ? 속성(Property) 와 함수(function) 를 하나로 묶은 자료
        - 변수명.속성명     : 객체내 속성값 호출
        - 변수명.속성명()   : 객체내 함수 호출
            console(객체).log)(함수) : console (내장)객체내 log 함수 호출
    ...
    [ DOM객체 ]
        ...
        3. 내장객체내 주요 속성
            ...
            3. document.querySelectorAll('선택자')  : HTML에 지정한 선택자를 모두 JS객체로 *반환*하는 함수
            *선택자 : 1.마크업명 2. .클래스명 3. #id명   
            ...
            7. dom객체.style.CSS 속성명 =            : 마크업내 style(CSS) 요소 추가
            8. dom객체.classList                     : 마크업내 적용된 class 들 반환
                dom객체.classList.add('클래스명')     : 마크업내 클래스 추가
                dom객체.classsList.remove('클래스명') : 마크업내 클래스 삭제
...
// 매개변수 : x , 반환값 : x , 로직 : 콘솔출력
// 로직 : 아래참고
function func1(){ // 함수 선언
    // 1. 함수실행테스트출력
     console.log('--- func1 exe ---'); 
    // 2. input 마크업을 JS dom객체를 가져오기
